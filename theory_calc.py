...
class AuctionGeneratorTheory:
    ...
    def evaluate_round_per_group(self):
        ...
        for i in self.groups:
            ...
            no_rym_first_losing_proxy[str(i)] = dict(
                sorted(bids.items(), key=itemgetter(1))[self.demand:self.demand + 1])
        ...
        for i in self.groups:
            bids = {}
            for ii in self.groups[i]:
                bids[str(ii)] = self.players_dic[ii].parameters["lcoe"] / self.players_dic[ii].parameters[
                    "correction_factor"]
    ...
    def store_round_results(self):
        for i in self.groups:
            ...
            for ii in self.rym_winning_bids_proxy[i]:
                ...
                rym_profit += (rym_marginal * self.players_dic[ii].parameters["correction_factor"] -
                               self.players_dic[ii].parameters["lcoe"]) \
                              * (self.players_dic[ii].parameters["production"] / rym_production)
    ...
    def export_everything(self, name="default_export"):
        writer = pd.ExcelWriter("results/{0}.xlsx".format(name), engine="xlsxwriter")
